...
# MY_ADDR="2881 Southeast 48th Avenue, Portland, Oregon"
...
def process_location(API_KEY):
    ...
    location = tools.get_coordinates(full_address)
    pois, campsites = data_compile.get_data(location,API_KEY)
    ...
    if location:
        ...
        folium.Marker(map_center, popup=full_address, icon=folium.Icon(color="blue")).add_to(map_obj)
        ...
        for idx, point in enumerate(pois):
            folium.Marker(point, popup=point['name'], icon=folium.Icon(color="green")).add_to(map_obj)
